	   ...
	   var confirmPopup = $ionicPopup.confirm({
		...
		buttons: [
			...
			},
			{
				...
				onTap: function(e){
					angular.forEach($scope.messages, function(value){
						if(value.selected){ Messages($localStorage.userLogin.id).delete(value.$id) }
					});
					location.reload();
				}
			}
...
.controller('messagesDetail', function($scope, $ionicScrollDelegate, $ionicTabsDelegate, $ionicSideMenuDelegate, $timeout, $ionicPopup, IonicClosePopupService, Notification, Block, Messages, Camera, DetailMessages, User, $localStorage, $state, $stateParams){
	$scope.showLoading('Loading...');
