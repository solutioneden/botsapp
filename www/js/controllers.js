angular.module('starter.controllers', ['ionic.closePopup'])
        ...
        buttons: [
          ...
          },
          {
            ...
            onTap: function (e) {
              angular.forEach($scope.messages, function (value) {
                if (value.selected) {
                  Messages($localStorage.userLogin.id).delete(value.$id)
                }
              });
              location.reload();
            }
  ...
  .controller('messagesDetail', function ($scope, $ionicScrollDelegate, $ionicTabsDelegate, $ionicSideMenuDelegate, $timeout, $ionicPopup, IonicClosePopupService, Notification, Block, Messages, Camera, DetailMessages, User, $localStorage, $state, $stateParams, $ionicModal) {
    ...
    $scope.openModal = function (title, content, link) {
      $scope.detail_modal = {};
      $scope.detail_modal.title = title;
      $scope.detail_modal.content = content;
      $scope.detail_modal.link = link;
      $scope.modal.show();
    };
    ...
    $scope.presetMessages = [
      {
        "id": "1",
        "conversationId": "1000000000",
        // "created": "2016-09-16T21:20:12.274Z",
        "from": "1",
        // "text": "Hi " + $scope.Me.name + ", what can i do for you?",
      ...
      },
      {
        "id": "2",
        "conversationId": "1000000000",
        // "created": "2016-09-16T21:30:12.274Z",
        "from": "1",
        // "text": "If you need to get a replacement card, please send us a secure E-message via online banking where we can assist you with a replacement. Alternatively you can go to your local branch to order a replacement card.",
        "content": "If you need to get a replacement card, please send us a secure E-message via online banking where we can assist you with a replacement. Alternatively you can go to your local branch to order a replacement card.",
        "channelData": {},
        "images": [
          "string"
        ],
      ...
      },
      {
        "id": "3",
        "conversationId": "1000000000",
        // "created": "2016-09-16T21:40:12.274Z",
        "from": "1",
        // "text": "You can easily update your home address online or by visiting us in branch. Please see our Change of Address support page for further information. Or update by login your account online.",
        "content": "You can easily update your home address online or by visiting us in branch. Please see our Change of Address support page for further information. Or update by login your account online.",
        "channelData": {
          "telephone": "tel:+441212888810",
          "online": "https://www.hsbc.co.uk"
        },
        "images": [
          "string"
        ],
      ...
      },
      {
        "id": "4",
        "conversationId": "1000000000",
        // "created": "2016-09-16T21:50:12.274Z",
        "from": "1",
        // "text": "Please select one for the following offers you are interested in.",
        "content": "Please select one for the following offers you are interested in.",
        "channelData": {
          "car": {
            "icon": "fa-car",
            "title": "Mercedes C63",
            "content": "Find out the latest new car offers and attractive deals from Mercedes-Benz. Search through all of the new vehicle offers and enquire at your local Retailer.",
            "link": "http://www.mercedes-benz.co.uk/content/unitedkingdom/mpc/mpc_unitedkingdom_website/en/home_mpc/passengercars.html"
          },
          "mortgage": {
            "icon": "fa-money",
            "title": "House in Central London",
            "content": "7 bedroom terraced house for sale. Eaton Square is found at the heart of the internationally renowned area of Belgravia, with all the many amenities of Knightsbridge, Sloane Street and Chelsea being close to hand. There are good transport links with fast access to Gatwick from Victoria Station (approximately 0.4 miles away), Sloane Square is approximately 0.4 miles away and Hyde Park is approximately 0.8 miles away which offers walks and other outside recreational activities (all distances and times are approximate).",
            "link": "http://www.rightmove.co.uk/property-for-sale/property-54934451.html"
          },
          "loan": {
            "icon": "fa-dollar",
            "title": "3.3% APR Representative for loans",
            "content": "3.3% APR Representative for loans between £7,000 and £15,000. Now also available to new HSBC customers - We'll offer you a rate based on our assessment of your personal financial circumstances. Interest rates may change at any time but the interest rate for your loan, once offered and accepted, will be fixed.",
            "link": "http://www.hsbc.co.uk/1/2/loans/personal-loan"
          },
          "investment": {
            "icon": "fa-file",
            "title": "Research, buy, and track investments funds via our Global Investment Centre",
            "content": "Our Global Investment Centre is our investments service through which you can open and invest in a Funds portfolio and ISA Funds portfolio, giving you access to investment funds from HSBC and a wide range of other providers.",
            "link": "https://investments.hsbc.co.uk/product/206/investment-funds-online"
          }
        },
        "images": [
          "string"
      ...
      },
      {
        ...
        // "created": "2016-09-16T21:40:12.274Z",
        "from": "1",
        // "text": "Sorry, we can't find any answer to your question, please contact our support below.",
        "content": "Sorry, we can't find any answer to your question, please contact our support below.",
        "channelData": {
          "telephone": "tel:+441212888810",
          "online": "https://www.hsbc.co.uk"
        },
      ...
      if (!$scope.Block.$value) {
        ...
        $scope.sendText = function () {
          if (angular.isDefined($scope.inputText.content) && angular.isString($scope.inputText.content)) {
            ...
            var message_id = 4;
            angular.forEach($scope.messageReaction, function (a_reaction) {
              angular.forEach(a_reaction.keywords, function (a_keyword) {
                if ($scope.Messages.content.toLowerCase().indexOf(a_keyword) > -1) {
                  message_id = a_reaction.message_id;
                }
              });
            });

            $scope.presetMessages[message_id].created = now;
            if (message_id == 0) {
              $scope.presetMessages[message_id].content = 'Hi ' + $scope.Me.name.$value + ', what can i do for you?';
            }
            DetailMessages($stateParams.id).post($localStorage.userLogin.id, $scope.presetMessages[message_id]);
  ...
  .controller('contactsSearch', function ($scope, $state, $stateParams, User) {
    $scope.showLoading("Loading...");
  ...
  .controller('contactsNearby', function ($scope, $http, $ionicModal, $ionicPopup, User) {
    ...
    $scope.updateNearby = function () {
      ...
      $scope.iNearby.$loaded(function () {
        ...
        angular.forEach($scope.iNearby, function (value) {
          ...
          if (value.age >= $scope.settings.fromage && value.age <= $scope.settings.toage) $scope.nearby.push(value);
        });
        $scope.hideLoading();
      });
    };
  ...
  .controller('groupCtrl', function ($scope, $http, IonicClosePopupService, UserGroups, Groups, User, $ionicPopup, $localStorage) {
    $scope.showLoading("Loading...");
    ...
    $scope.showPopupMenuGroup = function (id, title) {
      var name = '';
      for (var i = 0; i <= 2; i++) {
      ...
      var popupMenuGroup = $ionicPopup.show({
        ...
        buttons: [
          {
            text: 'Leave group', type: 'button-clear', onTap: function (e) {
    ...
    $scope.createGroup = function () {
      if ($scope.selectedCount > 0) {
        ...
        $scope.last.$loaded(function () {
          ...
          Groups().create($scope.last, $localStorage.userLogin.id, $scope.selectedCount, $scope.contactsSelected);
  ...
  .controller('groupDetail', function ($scope, $state, $localStorage, $ionicModal, $ionicTabsDelegate, $timeout, $ionicScrollDelegate, User, Groups, DetailGroups, Camera, $stateParams) {
    $scope.contentBottom = '100px';
    $scope.showLoading('Loading...');
